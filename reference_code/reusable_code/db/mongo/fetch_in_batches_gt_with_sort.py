...
def batch_fetch_using_sort(db, collection_name,
    ...
    <sort_by_field> and take the first document from the result set 
    2. Now add to <find_query> the condition <sort_by_field> to be > X, where X is
    the value taken from the <sort_by_field> of the fetched document. 
    3. Limit the count of the fetched documents and while data is fetched 
    replace the used value for X with the last fetched document's <sort_by_field>

    !!! ATTENTION !!!
        <sort_by_field> must be unique ($gt is used), and is required
        to exist in all documents
    ...
    if not matching_docs_count:
        return
    docs = retry_run(query, 1)
    if not docs:
        LOG.warning('There were documents matching the '
                    'filter but now nothing is found')
        return

    yield docs
    find_query[sort_by_field] = {'$gt': docs[0][sort_by_field]}
